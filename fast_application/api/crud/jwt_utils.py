...
def encode_jwt(
    ...
    encode = jwt.encode(to_encode, private_key, algorithm)
    return encode
...
async def _get_current_payload(
    ...
    try:
        ...
        if payload[Token.field] != Token.access:
            raise HTTPException(
                ...
                detail=f"token_type {payload[Token.field]}, expected token_type {Token.access}",
            )
...
async def _get_payload_refresh_token(
    token: HTTPAuthorizationCredentials = Depends(http_bearer),
) -> dict:
    token = token.credentials
    try:
        ...
        if payload[Token.field] != Token.refresh:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail=f"token_type {payload[Token.field]}, expected token_type {Token.refresh}",
            )
    except InvalidTokenError as e:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="token invalid"
