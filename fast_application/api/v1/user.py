...
@router.get('/me')
async def get_user(payload:dict=Depends(_get_current_payload), session:AsyncSession=Depends(db_helper.session_getter)):
    current_user = await _get_current_user(session=session, payload=payload)
    return current_user

@router.get('/all')
async def get_all_users(payload:dict=Depends(_get_current_payload), session:AsyncSession=Depends(db_helper.session_getter)):
